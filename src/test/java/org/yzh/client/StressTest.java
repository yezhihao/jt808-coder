...
public class StressTest {
    ...
    public static void main(String[] args) {
        ...
        JTHandler handler = new JTHandler();

        TCPClient[] clients = new TCPClient[Total];
        for (int i = 0; i < Total; i++) {
            String id = String.valueOf(i);
            clients[i] = new TCPClient(id, JTClient.TCPConfig, handler).start();
            clients[i].writeObject(T0100(id));
        }

        while (true) {
            String strTime = yyMMddHHmmss.format(deviceTime);
            deviceTime = deviceTime.plusSeconds(1L);

            for (int i = 0; i < Total; i++) {
                clients[i].writeObject(T0200(clients[i].name, strTime));
            }
            try {
